...
From there, the system hands control to the `parallel_lang_agent`, which acts as a coordinator for specialised language agents. This agent branches the request to three expert sub-agents:
...
Each of these expert agents sends its responses back into the architecture’s main loop, ensuring that all outputs are refined and integrated consistently before being delivered to the user. The closed-loop structure allows the system to adapt dynamically, maintain coherence, and personalise the user’s learning experience in real time.
...
## To-do
* Implement content compaction to learn user's language level. 
* Relation database to record user's points